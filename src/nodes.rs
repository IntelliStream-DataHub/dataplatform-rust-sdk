...
/// which is the one field a `Resource` always serializes and the shared `NodeModel` does not carry.
///
/// Besides `Resource` itself that is `Asset` alone. The api keeps `isRoot` on the two subclasses
/// that have a meaning for it and off every other — `Function`'s own javadoc says it "deliberately
/// does not extend Resource, which would hand it isRoot, geoLocation, valueType and elementId —
/// four fields a function has no meaning for". This list mirrors the SDK's own types exactly:
/// [`Asset`] and [`Resource`] are the only variants here with an `is_root` field, and `TimeSeries`,
/// `Dataset`, `Policy` and [`crate::functions::Function`] have none.
const RESOURCE_COMPATIBLE_LABELS: [NodeType; 2] = [NodeType::Resource, NodeType::Asset];
...
///   putting its label on a bare [`Resource`] keeps working — but only for `ASSET`, the one type
///   that can accept a resource's fields. [`Resource`] carries `is_root`, a plain `bool` with no
///   `skip_serializing_if`, so every body it builds names `isRoot`; the api keeps that field on
///   the two subclasses that legally have it (`Resource`, `Asset`) and off every other
///   (`Timeseries`, `Function`, `DataSetModel`, `Policy`), "which keeps illegal combinations
///   unrepresentable". Labelling a bare [`Resource`] with one of those four therefore cannot
...
mod tests {
    ...
    fn a_resource_may_be_labelled_as_a_type_that_accepts_is_root() {
        for label in ["ASSET", "asset", "RESOURCE"] {
            let mut r = Resource::new();
    ...
    fn a_resource_labelled_as_a_type_without_is_root_is_refused() {
        for label in ["DATASET", "dataset", "POLICY", "policy", "TIMESERIES", "FUNCTION"] {
            let mut r = Resource::new();
            r.external_id = "probe".to_string();
            r.labels = Some(vec![label.to_string()]);
