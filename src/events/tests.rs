use crate::events::filters::{BasicEventFilter, EventFilter, TimeFilter};
...
async fn find_events_by_external_ids(api_service: &ApiService, external_ids: Vec<&str>) {
    let result = api_service
        .events
        .by_ids(&IdAndExtIdCollection::from_external_id_vec(external_ids))
        .await;
    match result {
        Ok(events) => {
...
async fn test_event_filter() {
    ...
    let ext_ids = test_events
        .iter()
        .map(|e| e.external_id.as_ref())
        .collect::<Vec<&str>>();
    delete_events(&api_service, ext_ids).await;
