...
mod tests {
    ...
    async fn test_datapoints_binary() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // validate_daily_avg checks each daily average against the source file, which says the
        // binary path stored the right values. Comparing avg/min/max with a JSON-ingested twin
        // on top says the two paths store the *same* values, which is what this test is for.
