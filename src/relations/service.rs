...
impl EdgesService {
    ...
    ///
    /// **A name that already exists is a 409 `duplicate`**, like [`create`](Self::create) on a
    /// duplicate edge. The batch is all-or-nothing: one existing name rolls back the new types
    /// beside it, so resend without the duplicate. (It used to be a 200 with an empty body, the
    /// unique-hash collision surfacing only at commit; `test_duplicate_relationship_type_conflicts`
    /// pins the 409 for both cases.)
