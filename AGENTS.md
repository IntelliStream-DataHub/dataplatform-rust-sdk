...
- **Time windows are inclusive at both ends.** `TimeFilter` backs `createdTime`, `lastUpdatedTime` and `eventTime` alike, and a row landing exactly on `max` is returned. `eventTime` alone used to be exclusive there until the api made it `<=` like the other two — a day boundary written as `...T00:00:00Z` was dropping the event on it. Worth stating because the natural Rust spelling of a window, `min..max`, is half-open, which is how `events::tests::test_event_filter` came to expect one event fewer than it got.
