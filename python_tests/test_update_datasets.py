...
def test_scalar_set_value(sync_client, new_dataset, field, value, attr):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, **{field: intellistream_datahub_sdk.FieldStr(value=value)}
    ))
...
def test_external_id_set_value(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, external_id=intellistream_datahub_sdk.FieldStr(value=new_ext)
    ))

    found = poll_until(
        lambda: sync_client.datasets.filter(
...
def test_duplicate_external_id_is_rejected(sync_client, new_dataset):
    ...
    with pytest.raises(intellistream_datahub_sdk.DataHubException):
        sync_client.datasets.update([intellistream_datahub_sdk.DatasetUpdate(
            other.external_id, external_id=intellistream_datahub_sdk.FieldStr(value=taken.external_id)
        )])
...
def test_name_below_the_minimum_length_is_rejected(sync_client, new_dataset):
    ...
    with pytest.raises(intellistream_datahub_sdk.DataHubException):
        sync_client.datasets.update([intellistream_datahub_sdk.DatasetUpdate(
            dataset.external_id, name=intellistream_datahub_sdk.FieldStr(value="ab")
        )])
...
def test_description_set_null(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, description=intellistream_datahub_sdk.FieldStr(set_null=True)
    ))
...
def test_metadata_set_replaces_whole_map(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, metadata=intellistream_datahub_sdk.MapField.set({"only": "9"})
    ))
...
def test_metadata_add_merges_and_overwrites(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id,
        metadata=intellistream_datahub_sdk.MapField.delta(add={"overwrite": "new", "added": "2"}),
    ))
    md = updated.metadata or {}
...
def test_metadata_remove_keys(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, metadata=intellistream_datahub_sdk.MapField.delta(remove=["b", "c"])
    ))
    md = updated.metadata or {}
    assert md.get("a") == "1"
...
def test_metadata_cleared_by_an_empty_set(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, metadata=intellistream_datahub_sdk.MapField.set({})
    ))
...
def test_labels_add(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.delta(add=["CURATED"])
    ))
...
def test_labels_remove(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.delta(add=["CURATED", "DRAFT"])
    ))
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.delta(remove=["DRAFT"])
    ))
    assert _labels_of(sync_client, dataset.external_id) == ["CURATED", "DATASET"]
...
def test_labels_set_replaces_and_keeps_the_type_label(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.delta(add=["OLD"])
    ))
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.set(["FRESH"])
    ))
...
def test_type_label_cannot_be_removed(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, labels=intellistream_datahub_sdk.ListFieldStr.delta(remove=["DATASET"])
    ))
...
def test_update_persists_beyond_the_echo(sync_client, new_dataset):
    ...
    _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.external_id, description=intellistream_datahub_sdk.FieldStr(value="after")
    ))
...
def test_update_targeting_by_numeric_id(sync_client, new_dataset):
    ...
    updated = _apply(sync_client, intellistream_datahub_sdk.DatasetUpdate(
        dataset.id, description=intellistream_datahub_sdk.FieldStr(value="by numeric id")
    ))
