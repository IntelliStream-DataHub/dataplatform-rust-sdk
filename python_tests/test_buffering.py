...
def _unreachable_buffered_client(buffer_dir: Path) -> "intellistream_datahub_sdk.DataHubClient":
...
def test_event_buffers_to_disk_with_uuid_v7(tmp_path):
    ...
    result = client.events.create(
        [intellistream_datahub_sdk.Event(type="test", external_id=unique_id("buffer_event"), event_time=datetime.now(timezone.utc))]
    )
    assert result == []
...
def test_event_without_event_time_is_rejected():
    ...
    with pytest.raises(TypeError):
        intellistream_datahub_sdk.Event(type="test", external_id="py_no_time_event")

    ev = intellistream_datahub_sdk.Event(type="test", external_id="py_no_time_event", event_time=datetime.now(timezone.utc)
    )
    with pytest.raises(TypeError):
        ev.event_time = None
...
def test_live_event_gets_uuid_v7():
    ...
    try:
        created = client.events.create(
            [intellistream_datahub_sdk.Event(type="test", external_id=ext_id, event_time=datetime.now(timezone.utc))]
        )
