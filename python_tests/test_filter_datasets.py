...
The dataset filter is now exactly the shared node criteria, *minus* ``dataSetIds``: a data set is
...
* the retirement of ``externalIdPrefix``, folded into ``externalIds`` as a trailing wildcard — one
  field that now does exact lookup, prefix, suffix and contains, and can be given more than once;
* the retirement of the singular ``source`` in favour of the ``sources`` pattern list;
...
def flt(sync_client, prefix):
    ...
    def _filter(limit=None, **criteria):
        ...
        form = datahub_sdk.DatasetFilter(datahub_sdk.BasicDatasetFilter(**criteria), limit=limit)
...
def test_external_ids_do_exact_lookup_and_prefix_search_in_one_field(flt, datasets, both, prefix):
    parent, child = datasets
    assert flt(external_ids=[parent.external_id]) == {parent.external_id}
    assert flt(external_ids=[f"{prefix}*"]) == both
    assert flt(external_ids=[f"{prefix}_ds_%"]) == both, "`%` is a wildcard too"
...
def test_external_ids_can_be_given_more_than_once(flt, datasets, both, token):
    ...
    """
    parent, child = datasets
    assert flt(external_ids=[parent.external_id, f"*{token}_ds_child"]) == both


def test_a_suffix_and_contains_search(flt, datasets, prefix):
    parent, child = datasets
    assert flt(external_ids=["*_ds_parent"]) >= {parent.external_id}
    assert flt(external_ids=[f"*{prefix[-8:]}_ds_*"]) == {parent.external_id, child.external_id}
...
def test_underscore_is_literal(flt, datasets, prefix):
    ...
    assert flt(external_ids=[parent.external_id]) == {parent.external_id}
    assert flt(external_ids=[parent.external_id.replace("_ds_", "Xds_")]) == set()


def test_external_ids_are_case_insensitive(flt, datasets):
    parent, _child = datasets
    assert flt(external_ids=[parent.external_id.upper()]) == {parent.external_id}
...
def test_names_match_as_patterns_and_or_together(flt, datasets, both, token):
    parent, child = datasets
    assert flt(names=[f"Filter Parent {token}"]) == {parent.external_id}
    assert flt(names=[f"Filter * {token}"]) == both
    assert flt(names=[f"Filter Parent {token}", f"Filter Child {token}"]) == both
    assert flt(names=["no such name"]) == set()


def test_ids_match_exactly(flt, datasets, both):
    parent, child = datasets
    assert flt(ids=[parent.id]) == {parent.external_id}
    assert flt(ids=[parent.id, child.id]) == both
...
def test_the_retired_flags_are_not_accepted(sync_client):
    ...
    for retired in ["write_protected", "deactivated"]:
        with pytest.raises(TypeError):
            datahub_sdk.BasicDatasetFilter(**{retired: False})
...
def test_an_argument_free_filter_returns_everything(sync_client, datasets, both):
    ...
    from_filter = externals(sync_client.datasets.filter(datahub_sdk.DatasetFilter()))
...
def test_empty_and_blank_lists_place_no_restriction(flt, both, empty):
    assert flt(names=empty, sources=empty, labels=empty) == both


def test_none_valued_criteria_are_omitted(flt, both):
    assert flt(ids=None, names=None, sources=None, labels=None, metadata=None) == both


def test_garbled_criteria_match_nothing_without_erroring(flt):
    for garbage in ["!!!##$$^&()", "' OR 1=1 --", "\\", "%%%%_____", "😀"]:
        assert flt(names=[garbage]) == set(), f"{garbage!r} should match nothing"


def test_an_unmatchable_criterion_gives_an_empty_result_not_an_unfiltered_one(flt):
    assert flt(external_ids=["ds_does_not_exist_xyz"]) == set()
...
def test_limit_caps_the_page(sync_client, datasets, prefix):
    form = datahub_sdk.DatasetFilter(
        datahub_sdk.BasicDatasetFilter(external_ids=f"{prefix}*"), limit=1)
...
def test_a_limit_above_the_ceiling_is_refused(sync_client, prefix):
    form = datahub_sdk.DatasetFilter(
        datahub_sdk.BasicDatasetFilter(external_ids=f"{prefix}*"), limit=10_001)
...
def test_the_retired_criteria_are_not_accepted(sync_client):
    """``external_id_prefix`` and the singular ``source`` are gone from the binding as well as the
    wire, so a caller still passing them gets a ``TypeError`` rather than a query that quietly
    ignores them."""
    for retired, value in [("external_id_prefix", "sap_"), ("source", "sap")]:
        with pytest.raises(TypeError):
            datahub_sdk.BasicDatasetFilter(**{retired: value})


@pytest.mark.asyncio
async def test_async_filter_matches_the_sync_one(async_client, sync_client, datasets, prefix):
    form = datahub_sdk.DatasetFilter(datahub_sdk.BasicDatasetFilter(external_ids=f"{prefix}*"))
