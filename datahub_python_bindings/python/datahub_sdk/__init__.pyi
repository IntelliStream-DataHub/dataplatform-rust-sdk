...
class DataHubClient:
    ...
    @property
    def policies(self) -> PoliciesServiceSync: ...
...
class AsyncDataHubClient:
    ...
    @property
    def policies(self) -> PoliciesServiceAsync: ...
...
class PolicyScope:
    ...
    TENANT_ONLY: PolicyScope
    DATASET_ONLY: PolicyScope
    TENANT_WITH_DATASET_OVERRIDE: PolicyScope


class PolicyType:
    """The policy templates the server offers."""

    SECURITY_POLICY: PolicyType
    ENCRYPTION_POLICY: PolicyType
    MASKING_POLICY: PolicyType
    IS_WRITE_PROTECTED: PolicyType
    IS_READ_PROTECTED: PolicyType
    HAS_REQUIREMENT: PolicyType
    NAMING_CONVENTION: PolicyType

    @property
    def scope(self) -> PolicyScope: ...
    def can_attach_to_data_set(self) -> bool: ...
    def can_apply_tenant_wide(self) -> bool: ...


class PolicyDecision:
    OK: PolicyDecision
    WARNING: PolicyDecision
    NOT_OK: PolicyDecision


class Policy:
    """A policy node.

    Every type except ``NAMING_CONVENTION`` must name a ``data_set_id`` — see
    ``PolicyType.scope``. Creating one without it is refused with status 400.
    """
    ...
    def labels(self) -> list[str]: ...
    @property
    def type(self) -> PolicyType | None: ...
    @type.setter
    def type(self, value: PolicyType | None) -> None: ...
    @property
    def value(self) -> Any | None: ...
    @property
    def deactivated(self) -> bool: ...
    @deactivated.setter
    def deactivated(self, value: bool) -> None: ...
    @property
...
class NamingCheck:
    """The answer from ``policies.check_naming``. Only non-conforming ids appear."""
    ...
    @property
    def rejections(self) -> list[PolicyFinding]: ...
    def __len__(self) -> int: ...
    def __bool__(self) -> bool: ...


class NamingCheckForm:
    """Candidate ids to run the naming policy over.

    ``external_ids`` must not be empty. ``names`` is optional positional labels for those ids —
    supply exactly as many as there are ids, or none.
    """
...
PolicyIdentifiable = Union[Policy, int, str]


class PoliciesServiceSync:
    def list(self) -> list[Policy]: ...
    def types(self) -> list[Policy]: ...
    def get(self, id: int) -> Policy | None: ...
    def create(self, input: list[Policy]) -> list[Policy]: ...
    def update(self, input: list[Policy]) -> list[Policy]: ...
    def delete(self, input: list[PolicyIdentifiable]) -> None: ...
    def check_naming(self, form: NamingCheckForm) -> NamingCheck: ...


class PoliciesServiceAsync:
    async def list(self) -> list[Policy]: ...
    async def types(self) -> list[Policy]: ...
    async def get(self, id: int) -> Policy | None: ...
    async def create(self, input: list[Policy]) -> list[Policy]: ...
    async def update(self, input: list[Policy]) -> list[Policy]: ...
    async def delete(self, input: list[PolicyIdentifiable]) -> None: ...
    async def check_naming(self, form: NamingCheckForm) -> NamingCheck: ...
