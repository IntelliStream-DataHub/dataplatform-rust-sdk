...
use crate::{DatahubIdentity, Identifyable, PyIdCollection, PySearchAndFilterForm};
use crate::dataset::{DatasetIdentifiable, PyDataset};
...
impl PyDatasetsServiceAsync {
    fn list<'p>(&self, py: Python<'p>) -> PyResult<Bound<'p, PyAny>> {
        ...
        future_into_py(py, async move {
            let result = service.datasets.list().await.map_err(|e| {
                PyException::new_err(e.get_message())
            })?;

            let py_ts: Vec<PyDataset> = result.get_items().iter().map(|ts| PyDataset { inner: ts.clone() }).collect();
            Ok(py_ts)
        })
    }
    ...
    fn create<'p>(&self, py: Python<'p>,input: Vec<PyDataset>) -> PyResult<Bound<'p, PyAny>> {
        ...
        let service = self.api_service.clone();
        future_into_py(py, async move {
            let result = service.datasets.create(&datasets).await.map_err(|e| {
                PyException::new_err(e.get_message())
            })?;
    ...
    fn by_ids<'p>(&self, py: Python<'p>, input: Vec<DatasetIdentifiable>) -> PyResult<Bound<'p, PyAny>> {
        let service = self.api_service.clone();
        let input_ids = input
            .into_iter()
            .map(IdAndExtId::from)
    ...
    fn delete<'p>(&self, py: Python<'p>,input: Vec<DatasetIdentifiable>) -> PyResult<Bound<'p, PyAny>> {
        let service = self.api_service.clone();
        let input_ids = input
            .into_iter()
            .map(IdAndExtId::from)
        ...
        future_into_py(py, async move {
            let result = service.datasets.delete(&input_ids).await.map_err(|e| {
                PyException::new_err(e.get_message())
            })?;
