...
use crate::events::{PyBasicEventFilter, PyEvent, PyEventFilter, PyEventIdCollection, PyTimeFilter};
...
#[pymethods]
impl PyDataset {
    /// Create a new Python dataset entity.
    ///
    #[new]
    #[pyo3(signature=(
        external_id,
...
impl DatasetIdentifiable {
    ...
    pub fn external_id(&self) -> Option<&str> { // todo! decide if we want to return Option<&str> or &str would require IdAndExtId to be changed to always force external_id to be Some
        match self {
...
impl From<DatasetIdentifiable> for IdAndExtId{
    fn from(value: DatasetIdentifiable) -> Self {
        match value {
            DatasetIdentifiable::IdCollection(id) => Self{ id:  id.id(), external_id:  id.external_id().map(|id|id.to_string())},
            DatasetIdentifiable::Dataset(event) => Self{ id : event.id(),  external_id: Some(event.external_id().to_string())},
            DatasetIdentifiable::ExternalId(id) => Self{ id: None, external_id: Some(id.to_string())}
        }
    }
}
