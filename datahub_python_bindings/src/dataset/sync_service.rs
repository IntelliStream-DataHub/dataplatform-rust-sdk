...
impl PyDatasetsServiceSync {
    ...
    fn create<'py>(&self, py: Python<'py>,input: Vec<PyDataset>) -> PyResult<Vec<PyDataset>> {
        ...
        let result = py.detach(|| {
            self.runtime
                .block_on(service.datasets.create(&datasets))
        });

        let result = result.map_err(|e| PyException::new_err(e.get_message()))?;

        let py_res: Vec<PyDataset> = result.get_items().iter().map(|ts| PyDataset { inner: ts.clone() }).collect();
        Ok(py_res)
    }

    fn by_ids<'py>(&self, py: Python<'py>, input: Vec<DatasetIdentifiable>) -> PyResult<Vec<PyDataset>> {
        let service = self.api_service.clone();
        let input_ids = input
            .into_iter()
            .map(IdAndExtId::from)
        ...
        let result = py.detach(|| {
            self.runtime
                .block_on(service.datasets.by_ids(&input_ids))
        });
    ...
    fn delete<'py>(&self, py: Python<'py>,input: Vec<DatasetIdentifiable>) -> PyResult<()> {
        ...
        py.detach(|| {
            self.runtime
                .block_on(service.datasets.delete(&input_ids)).map_err(|e| PyException::new_err(e.get_message()))
        })?;
